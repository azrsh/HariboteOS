...
void init_pit(void) {
  ...
  io_out8(
      PIT_COUNT0,
      0x2e); // PITの設定値の上位8bit(併せて0x2e9c=119318で、タイマ割込みは100Hzになる)
  timerControl.count = 0;
  for (i = 0; i < MAX_TIMER; i++) {
    timerControl.timers0[i].flags = TIMER_FLAGS_FREE;
  }
  timer =
      timer_allocate(); //最後尾にINT_MAXをtimeoutとする(つまり永遠に使われないと期待できる)要素を置き、状態を減らす。この手法を番兵という
...
void timer_free(struct TIMER *timer) { timer->flags = TIMER_FLAGS_FREE; }
