...
struct TASK *task_init(struct MEMORYMANAGER *memoryManager) {
  ...
  taskControl = (struct TASKCONTROL *)memorymanager_allocate_4k(
      memoryManager, sizeof(struct TASKCONTROL));
  for (i = 0; i < MAX_TASKS; i++) {
    taskControl->tasks0[i].flags = 0;
    taskControl->tasks0[i].selector = (TASK_GDT0 + i) * 8;
    set_segment_descriptor(gdt + TASK_GDT0 + i, 103,
                           (int)&taskControl->tasks0[i].tss, AR_TSS32);
  }
  ...
  idle = task_allocate();
  idle->tss.esp =
      memorymanager_allocate_4k(memoryManager, 64 * 1024) + 64 * 1024;
