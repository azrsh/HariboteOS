...
void set_segment_descriptor(struct SEGMENT_DESCRIPTOR *segmentDescriptor,
                            unsigned int limit, int base, int access_right);
void set_gate_descriptor(struct GATE_DESCRIPTOR *gateDescriptor, int offset,
...
#define MEMMAN_ADDR                                                            \
  0x003c0000 //メモリマネージャの先頭アドレス。ここから32KBがメモリマネージャの構造体
struct FREEINFO {
...
unsigned int memorymanager_allocate(struct MEMORYMANAGER *memorymanager,
                                    unsigned int size);
int memorymanager_free(struct MEMORYMANAGER *memorymanager,
                       unsigned int address, unsigned int size);
unsigned int memorymanager_allocate_4k(struct MEMORYMANAGER *memorymanager,
                                       unsigned int size);
int memorymanager_free_4k(struct MEMORYMANAGER *memorymanager,
                          unsigned int address, unsigned int size);
...
void sheet_refresh(struct SHEET *sheet, int box0, int boxY0, int boxX1,
                   int boxY1);
