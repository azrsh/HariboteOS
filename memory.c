...
int memorymanager_free(struct MEMORYMANAGER *memorymanager,
  ...
  {
    if (memorymanager->freeInfo[i - 1].address +
            memorymanager->freeInfo[i - 1].size ==
        address) {
      memorymanager->freeInfo[i - 1].size +=
          size; //前の空き領域の終端と開放する領域の先頭が隣接しているので統合
      if (i < memorymanager->frees) //後ろに空き領域が存在するとき
      {
        if (address + size == memorymanager->freeInfo[i].address) {
          //開放する領域の終端と後ろの領域の先頭が隣接しているので統合する
          memorymanager->freeInfo[i - 1].size +=
              memorymanager->freeInfo[i].size;
  ...
  {
    if (address + size ==
        memorymanager->freeInfo[i]
            .address) //開放する領域の終端と後ろの領域の先頭が隣接しているので統合する
    {
